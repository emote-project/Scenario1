...
namespace CaseBasedController.Programs
{
    static public class CasePoolCodingProgram
    {
        ...
        static public CasePool EnercitiesDemoEmpathic()
        {
            ...
            IFeatureDetector aiPlannedActionDuringMayrTurnDetector = new AndFeatureDetector()
            {
                ...
                Detectors = new List<IFeatureDetector>()
                {
                    ...
                    aiPlannedAction,
                    new NotFeatureDetector() { WatchedDetector = newLevelMenuShowedBinaryFeatureDetector},
                    new NotFeatureDetector() { WatchedDetector = new AndFeatureDetector()
                        { 
                            Detectors = new List<IFeatureDetector>(){
                                isTurn1BinaryFeatureDetector,
        ...
        static public CasePool TestPool()
        {
            ...
            //casePool.Add(new Case(gameStartBinaryFeatureDetector, b_utt_tutorial) { Description = "Tell tutorial" });
            // #################################### GAME MECANICS
            casePool.Add(new Case(turnChangedToAiPlayerDetector, b_utt_turnchanged_self) { Description = "Say when turn changes to self" });
            casePool.Add(new Case(turnChangedToHumanPlayersDetector, b_utt_turnchanged_other) { Description = "Say when turn changes human players" });
            casePool.Add(new Case(aiPlannedActionDuringMayrTurnDetector, b_makeRecomGameMove) { Description = "Play recomended game move" });
            casePool.Add(new Case(aiPlayedGameActionDetector, b_commentAIGameAction) { Description = "Comment AI game action" });
            casePool.Add(new Case(newLevelInstantFeatureDetector, b_commentNewLevel) { Description = "Say when a new level is reached" });
            // ####################################################

            casePool.Add(new Case(aiTakingTooLongDuringAiTurn, b_commentAITakingTooLong) { Description = "Comment AI taking too long" });
            casePool.Add(new Case(isHumanIdleBinaryFeatureDetector, b_talkAboutRules) { Description = "Helps the human player in the early game if he doesn't act" });
