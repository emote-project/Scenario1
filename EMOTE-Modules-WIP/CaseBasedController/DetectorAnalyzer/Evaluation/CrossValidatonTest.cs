...
namespace DetectorAnalyzer.Evaluation
{
    public class CrossValidatonTest : ClassificationTest
    {
        ...
        protected override ClassificationPerformance Test(string filePath, ClassificationArffLoader firstLoader)
        {
            //collects the performance in all folds
            var performances = new List<ClassificationPerformance>();
            ...
            return ClassificationPerformance.GetAverage(performances);
        }

        private ClassificationPerformance RunFold(string filePath, int startInstance, int endInstance)
        {
