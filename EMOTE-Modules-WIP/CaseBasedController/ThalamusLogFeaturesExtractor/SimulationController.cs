...
namespace ThalamusLogFeautresExtractor
{
    public class SimulationController : IDisposable
    {
        ...
        public enum State { INITIALIZED, RUNNING, CANCELED, FINISHED, FILE_LOADED }

        const string OUTPUT_PATH = "../Output/";
        const string OUTPUT_FILE_NAME = "features";

        CasePool _casePool;
        List<string> _logFilesPath = new List<string>() {};

        FeaturesCollector _fc;
        LogDelegate _logHandler;
        CurrentProgressDelegate _currentProgressHandler;

        ControllerClient _client;
        State _state = State.INITIALIZED;
        double _progress = 0;
        string _loadedCasePoolPath = null;
        string _thalamusMessagesDLLsPath = "../ThalamusMessagesDLLs/";


                
        public SimulationController(string casePoolPath, string logFilesPath, string thalamusDLLsPath, LogDelegate logHandler = null, CurrentProgressDelegate progressHandler = null)
        {
            ...
            {
                throw new System.IO.FileNotFoundException("Can't find file at: " + logFilesPath);
            }
        ...
        public void Simulate(System.Threading.CancellationToken token)
        {
        ...
        public void SimulateAndAugment(System.Threading.CancellationToken token){
            string filesFolderPath = PathWithoutFilename(_logFilesPath[0]);
            foreach (string path in Directory.GetFiles(filesFolderPath, "*.arff")) File.Delete(path);
            ResetSimulationEnvironment();
            Log("=================================== RUNNING SIMULATION");
        ...
        private void PrepareAndRunSimulation(System.Threading.CancellationToken token, bool exportArffWithTime)
        {
            ...
            foreach (string logFilePath in _logFilesPath)
            {
                ...
                // ============== EXPORT THE ARFF
                string outputFileName = Path.ChangeExtension(logFilePath, "arff"); //OUTPUT_PATH+OUTPUT_FILE_NAME + fileIndex + ".arff";
        ...
        private void RunSimulation(List<LogEntry> thalamusLog, System.Threading.CancellationToken token, int totalRuns, int runNumber)
        {
        ...
        public void AugmentDetectorsInCasePool(CasePool casePool, StatisticsArffLoader statisticsAnalyzer)
        {
            ...
            {
                var result = casePool.GetPool().Values.Where(c => c.Description.Equals(detectorStatistics.Key)).Select(c => c.Detector);
                if (result.Count()<=0) continue;
                var detectorToWatch = result.First();
                
                var frequencyDetector = new FeatureFrequencyDetector(detectorToWatch,detectorStatistics.Value.Avg,detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.AVERAGE);
                frequencyDetector.Description = detectorToWatch.Description + "_AverageFrequency";
                casePool.Add(new Case(frequencyDetector, new EmptyBehaviour()) { Description = frequencyDetector.Description });
                
                var frequencyDetectorLTA = new FeatureFrequencyDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.LESS_THAN_AVERAGE);
                frequencyDetector.Description = detectorToWatch.Description + "_LessThanAverageFrequency";
                casePool.Add(new Case(frequencyDetectorLTA, new EmptyBehaviour()) { Description = frequencyDetector.Description });
                
                var frequencyDetectorMTA = new FeatureFrequencyDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.MORE_THAN_AVERAGE);
                frequencyDetector.Description = detectorToWatch.Description + "_MoreThanAverageFrequency";
                casePool.Add(new Case(frequencyDetectorMTA, new EmptyBehaviour()) { Description = frequencyDetector.Description });
            }
            foreach (var detectorStatistics in statisticsAnalyzer.TimeActiveStats) 
            {
                ...
                var activeTimeDetector = new FeatureActivationTimeDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.AVERAGE);
                activeTimeDetector.Description = detectorToWatch.Description + "_AverageActiveTime";
                casePool.Add(new Case(activeTimeDetector, new EmptyBehaviour()) { Description = activeTimeDetector.Description });

                var activeTimeDetectorLTA = new FeatureActivationTimeDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.LESS_THAN_AVERAGE);
                activeTimeDetector.Description = detectorToWatch.Description + "_LessThanAverageActiveTime";
                casePool.Add(new Case(activeTimeDetectorLTA, new EmptyBehaviour()) { Description = activeTimeDetector.Description });

                var activeTimeDetectorMTA = new FeatureActivationTimeDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureFrequencyDetector.ActivationMode.MORE_THAN_AVERAGE);
                activeTimeDetector.Description = detectorToWatch.Description + "_MoreThanAverageActiveTime";
                casePool.Add(new Case(activeTimeDetectorMTA, new EmptyBehaviour()) { Description = activeTimeDetector.Description });
            }
            ...
            foreach (var detectorStatistics in statisticsAnalyzer.TimeSinceLastActivationStats) 
            {
                ...
                var activeDelay = new FeatureActivationDelayDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureStatisticalPropertyDetector.ActivationMode.AVERAGE);
                ...
                var activeDelayLTA = new FeatureActivationDelayDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureStatisticalPropertyDetector.ActivationMode.AVERAGE);
                ...
                var activeDelayMTA = new FeatureActivationDelayDetector(detectorToWatch, detectorStatistics.Value.Avg, detectorStatistics.Value.StdDev, FeatureStatisticalPropertyDetector.ActivationMode.AVERAGE);
        ...
        private List<LogEntry> LoadLogFile(string logFilePath)
        {
            ...
            LogTool.LoadingAssemblyTypesErrorEvent -= LogTool_LoadingAssemblyTypesErrorEvent; // Avoid duplicated hanlders
            LogTool.LoadingAssemblyTypesErrorEvent += LogTool_LoadingAssemblyTypesErrorEvent;
            List<LogEntry> thalamusLog = LogTool.LoadThalamusLogEntries(logFilePath, Path.GetFullPath(_thalamusMessagesDLLsPath));//System.Reflection.Assembly.GetExecutingAssembly().Location));// Application.StartupPath+ "\\..\\ThalamusMessagesDLLs\\");          // TODO: Dynamically load this
        ...
        public void SetLogFiles(string folderPath)
        {
            ...
            _logFilesPath = _logFilesPath.Where(l => Path.GetExtension(l).Equals(".log")).ToList<string>();
            SetState(State.FILE_LOADED);
        }

        public string GetLoadedLogFile()
        {
            return (_logFilesPath.Count > 1 ? _logFilesPath.Count + " files in " + Path.GetDirectoryName(_logFilesPath.First()) : _logFilesPath.First()) ;
        }
        ...
        void fcc_NewFeaturesVector(object sender, NewFeaturesVectorEventArgs e)
        {
            string txt = MyTimer.GetCurrentTime().ToString() + " >> ";
        ...
        */
        ...
        void LogTool_LoadingTypesErrorEvent(object sender, LoadingInterfacesErrorEventArgs e)
        {
            string message = "Can't load following types:";
            foreach (string t in e.NotLoadedTypes)
            {
                message += System.Environment.NewLine + '\t' + t;
            }
            Log(message);
        }

        void LogTool_LoadingAssemblyErrorEvent(object sender, LoadingAssemblyErrorEventArgs e)
        {
            string message = "Can't load Assembly: "+e.NotLoadedAssembly;
            Log(message);
        }

        void LogTool_LoadingAssemblyTypesErrorEvent(object sender, LoadingAssemblyTypesEventArgs e)
        {
        ...
        public CasePool LoadCasePool(string path)
        {
            ...
            {
                Log("Error loading CasePool file! Can't load file " + System.IO.Path.GetFullPath(path));
                return null;
            }
        }
