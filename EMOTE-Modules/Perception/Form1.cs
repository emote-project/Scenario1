...
namespace Perception
{
    public partial class Form1 : Form
    {
        ...
        private void DisplayText(object sender, EventArgs e)
        {
            ...
            string[] q_temp1 = RxString.Split(',');
            if (qenable.Checked ==true)
            {    //packet,z,x,y,volts,temp,eda
                Qdata1 = (q_temp1.Length > 1 ? q_temp1[1] : "0") + "," + (q_temp1.Length > 2 ? q_temp1[2] : "0") + "," + (q_temp1.Length > 3 ? q_temp1[3] : "0") + "," + (q_temp1.Length > 5 ? q_temp1[5] : "0") + "," + (q_temp1.Length > 6 ? q_temp1[6] : "0");
                string[] tempp = Qdata1.Split(',');
                try
                {
                    qAccelZ1 = Convert.ToDouble(tempp[0]);
                    qAccelX1 = Convert.ToDouble(tempp[1]);
                    qAccelY1 = Convert.ToDouble(tempp[2]);
                    qtemp1 = Convert.ToDouble(tempp[3]);
                    qArousal1 = Convert.ToDouble(tempp[4]);
                }
                catch(Exception ddd){
                    qAccelZ1 = 0;
                    qAccelX1 = 0;
