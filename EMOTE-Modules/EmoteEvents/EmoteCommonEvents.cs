...
namespace EmoteCommonMessages
{
    ...
    public enum LearnerModelUpdateReason
    {
        ...
        AffectUpdate, // this is if the reason for the learner model update is due to a change in the affect perception module and not triggered by the task.
        ToolUse, // not a step answer attempt but due to a tool being used, this won't be sent at the moment but will do it soon. 
        Summary,
         Wrapup
     }
    ...
    public interface IFMLSpeechEvents : Thalamus.IPerception
    {
    ...
    public interface IEmoteActions : Thalamus.IAction
    {
	...
	{
        void Click(double x, double y);       // User clicking on a point on the screen. x and y representing the coordinates of the point in pixels
        void Zoom(double[] finger0, double[] finger1, double[] finger0Start, double[] finger1Start);      // each array contains the x and the y coordinate of each finger. The "start" represents the coordinates of the location where zoom gesture started
    ...
    public interface IApplicationActions : Thalamus.IAction
    {
	...
	public interface IMenuEvents : Thalamus.IPerception
	{
		void StartGameShow();
		void StartGameHide();
		void EndOfLevelShow();
    ...
    public interface IPerceptionEvents : Thalamus.IPerception
    {
        ...
        /// NOTE: user 1 is the one that, facing towards the camera, stands on the left of it; user 2 the one on the right. (In enercities: user 1: economist, user 2: environmentalist)
        ///       If a scenario with just one user is used than only user 1 values are used.
        /// </summary>
        /// <param name="au2User1">Outer Brow Raiser: 0=neutral, -1=fully lowered as a very sad face, +1=raised as in an expression of deep surprise</param>
        /// <param name="au4User1">Brow Lowerer: 0=neutral, -1=raised almost all the way, +1=fully lowered (to the limit of the eyes)</param>
        ...
        void PointingPosition(int userID,Hand hand, double X, double Y, double Z); //for publishing the position of the handtip
        ...
        void OKAOMessage(int userID, double smile, double confidence, double anger, double disgust, double fear, double joy, double sadness, double surprise, double neutral,string gazeDirection);
        ...
        // userID 1 or 2
        // Z,Y,X are the accelerometer values of the user's hand with values -5 to 5. it corresponds to the G force of the hand
        // X is up (+)/down (-)
        // Z is left (+)/right (-) with the big button closer to the right edge
        // Y is forward (+)/ backward (-)
        // Temp skin temperature in Celsius

        // EDA electroderma activity from 0 to 5 (didnt find the max value of this) microsiemens (a unit of conductance)

    }

    public class PerceptionLog : Thalamus.IPerception
    {
        /*
         *  09 March 2015
         */
        ...
        public PerceptionLog(double time, double faceUpDownDegrees, double faceLeftRightDegrees, double eyesUpdown, double eyesLeftRight, double headPositionY, double headPositionX, double closeRatioLeftEye, double closeRatioRightEye, double smile, double confidence, double anger, double disgust, double fear, double joy, double sadness, double surprise, double neutral, double gazeVectorX, double gazeVectorY, string gazeDirection)
        {
            this.time = time;
    ...
    public interface ISoundLocalizationEvents : Thalamus.IPerception
    {
    ...
    public interface IAffectPerceptionEvents : Thalamus.IPerception
    {     
    ...
    public interface ILearnerModelToAffectPerceptionEvents : Thalamus.IPerception
    {
    ...
    public interface ILearnerModelToS2IMPerceptionEvents : Thalamus.IPerception
    {
        //This event is fired after the learner model is updated with task information, but before the latest affect perception module is available.  affect perception is the main subscriber. 
        void learnerModelValueUpdate(string LearnerStateInfo_learnerState);
    }

    public interface ILearnerModelToIMEvents : Thalamus.IPerception
    {
        ...
        void learnerModelValueUpdateAfterAffectPerceptionUpdate(string LearnerStateInfo_learnerState , string AffectPerceptionInfo_AffectiveStates);

    }

    public interface ILearnerModelIdActions : Thalamus.IAction
    {
    ...
    {
        /// <summary>
        /// Comunicates when an utterance was used to the Learner Model. The LM will keep track of the used utterances in each session and return this history to the system every new one.
        /// </summary>
        /// <param name="id">The ID used in the thalamus messages to identify the utterance. It <b>differs</b> from the utterance.id which indentifies the single utterance inside its utterance library </param>
        /// <param name="Utterance_utterance">String resulting from the serialization of the class: <see cref="EmoteEvents.ComplexData.Utterance" /> </param>
        void UtteranceUsed(string id, string Utterance_utterance);
    }

    public interface ILearnerModelIdEvents : Thalamus.IPerception
    {
        ...
        void allUtterancesForParticipant(int participantId, string[] Utterance_utterances); // all existing student info
    }

    public interface ILearnerModelActions : Thalamus.IAction
    {
    ...
    public interface ILearnerModelMemoryEvents: Thalamus.IPerception
    {
    ...
    public interface Logs : Thalamus.IPerception
    {
        void PerceptionLog(double time, double faceUpDownDegrees, double faceLeftRightDegrees, double eyesUpdown, double eyesLeftRight, double headPositionY, double headPositionX, double closeRatioLeftEye, double closeRatioRightEye, double smile, double confidence, double anger, double disgust, double fear, double joy, double sadness, double surprise, double neutral, double gazeVectorX, double gazeVectorY, string gazeDirection);
    }
