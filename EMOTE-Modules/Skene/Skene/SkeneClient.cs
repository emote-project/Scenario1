...
namespace Skene
{
    ...
    public class SkeneClient : ThalamusClient, ISkene
    {
        ...
        void Thalamus.BML.IGazeEvents.GazeStarted(string id)
        {
            ...
            if (GazeManager.LastGazeTarget == GazeTarget.Person || GazeManager.LastGazeTarget == GazeTarget.Person2)
            {
        ...
        {
            if ((PersonGazeDirection.ContainsKey(1) && currentGazePerson == GazeTarget.Person && PersonGazeDirection[1] == GazeEnum.Robot) || (PersonGazeDirection.ContainsKey(2) && currentGazePerson == GazeTarget.Person2 && PersonGazeDirection[2] == GazeEnum.Robot))
            {
                if (!mutualGaze)
                {
        ...
        public class UtteranceHistoryReceivedEventArgs : EventArgs
        {
            ...
            public List<UtteranceHistoryItem> UtterancesHistory { get; set; }
        }
        ...
        public void QSensorMessage(int userID, double Z, double X, double Y, double Temp, double EDA)
        {
        }
        ...
        public void allUtterancesForParticipant(int participantId, string[] Utterance_utterances)
        {
            ...
            List<UtteranceHistoryItem> utterances = Utterance_utterances.Select(UtteranceHistoryItem.DeserializeFromJson).ToList();
        ...
        public void Start(string EmoteUser_user1, string EmoteUser_user2, string p1name, string p2name)
        {
        }
